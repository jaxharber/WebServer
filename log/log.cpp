#include <string.h>
#include <time.h>
#include <sys/time.h>
#include <stdarg.h>
#include "log.h"
#include <pthread.h>
using namespace std;

log::log()
{
    m_line_count = 0;
    m_is_async = false;
}
log::~log()
{
    if (log_file != NULL)
    {
        fclose(log_file);
    }
}
//日志创建、写入方式判断 异步需要设置阻塞队列的长度，同步不需要设置
bool log::init(const char* file_name, int close_flag, int log_buf_size, int split_lines, int max_queue_size)
{
    //采用异步写入
    if (max_queue_size >= 1)
    {
        m_is_async = true;
        //创建并设置阻塞队列长度
        m_log_queue = new block_queue<string>(max_queue_size);
        pthread_t tid;
        //flush_log_thread为回调函数,这里表示创建线程异步写日志
        pthread_create(&tid, NULL, flush_log_thread, NULL);
    }
    m_close_log = close_flag;
    //日志缓冲区长度
    m_log_buf_size = log_buf_size;
    m_buf = new char[m_log_buf_size];
    memset(m_buf, '\0', m_log_buf_size);
    //一份日志文件最大行数
    m_split_lines = split_lines;

    time_t tm = time(NULL);
    struct tm *sys_tm = localtime(&tm);
    struct tm my_tm = *sys_tm;
    m_today = my_tm.tm_mday;//记录当前天数（按天分类）
    //拼接日志文件名
    //从后往前找到第一个/的位置
    const char* p = strrchr(file_name, '/');
    char log_full_name[256] = {0};
    if (p == NULL)
    {
        // 无/则表示在当前工作目录下（"log.info"） 时间+日志名
        snprintf(log_full_name, 255, "%d_%02d_%02d_%s", my_tm.tm_year + 1900, my_tm.tm_mon + 1, my_tm.tm_mday, file_name);
    }
    else 
    {
        //绝对路径或者"./log.info"
        strcpy(log_name, p + 1);//文件名
        strncpy(dir_name, file_name,  p - file_name + 1);
        snprintf(log_full_name, 255, "%s%d_%02d_%02d_%s", dir_name, my_tm.tm_year + 1900, my_tm.tm_mon + 1, my_tm.tm_mday, log_name);
    }
    
    log_file = fopen(log_full_name, "a");
    
    if (log_file == NULL)
    {
        return false;
    } 
    return true;
}
//日志分级、分文件、格式化输出内容
void log::write_log(int level, const char* format, ...)
{
    struct timeval now = {0, 0};
    gettimeofday(&now, NULL);
    time_t t = now.tv_sec;
    struct tm *sys_tm = localtime(&t);//time_t-->tm转换
    struct tm my_tm = *sys_tm;

    char s[16] = {0};
    //日志分级
    switch(level)
    {
        case 0:
            strcpy(s, "[debug]:");
            break;
        case 1:
            strcpy(s, "[info]:");
            break;
        case 2:
            strcpy(s, "[warn]:");
            break;
        case 3:
            strcpy(s, "[error]:");
            break;
        default:
            strcpy(s, "[info]:");
            break;
    }

    //超行、按天分文件逻辑
    //开始写入一条日志信息
    m_mutex.lock();
    m_line_count++;
    //日志不是今天或写入的日志行数是最大行的倍数 创建新log文件
    if (m_today != my_tm.tm_mday || m_line_count % m_split_lines == 0)
    {
        char new_log[256] = {0};
        fflush(log_file);//刷新缓冲区
        fclose(log_file);
        char tail[16] = {0};
        snprintf(tail, 16, "%d_%02d_%02d_", my_tm.tm_year + 1900, my_tm.tm_mon + 1, my_tm.tm_mday);
        if (m_today != my_tm.tm_mday)
        {
            snprintf(new_log, 255, "%s%s%s", dir_name, tail, log_name);
            m_today = my_tm.tm_mday;
            m_line_count = 0;
        }
        else 
        {
            snprintf(new_log, 255, "%s%s%s.%lld", dir_name, tail, log_name, m_line_count / m_split_lines);
        }
        //创建新文件
        log_file = fopen(new_log, "a");
        //delete [] new_log;
    }
    m_mutex.unlock();

    va_list valst;
    //利用format参数获得可变参数列表存入valst
    va_start(valst, format);
    string log_str;
    m_mutex.lock();

    //写入内容格式：时间 + 内容
    //时间格式化，snprintf成功返回写字符的总数，其中不包括结尾的null字符
    int n = snprintf(m_buf, 48, "%d-%02d-%02d %02d:%02d:%02d.%06ld %s ",
                      my_tm.tm_year + 1900, my_tm.tm_mon + 1, my_tm.tm_mday,
                      my_tm.tm_hour, my_tm.tm_min, my_tm.tm_sec, now.tv_usec, s);

    //内容格式化，用于向字符串中打印数据、数据格式用户自定义，返回写入到字符数组str中的字符个数(不包含终止符)
    int m = vsnprintf(m_buf + n, m_log_buf_size - 1, format, valst);//用va_list替代可变参数
    m_buf[n + m] = '\n';
    m_buf[n + m + 1] = '\0';//完整的一行日志
    log_str = m_buf;
    m_mutex.unlock();

    //根据写入方式来向日志文件中写入日志
    if (m_is_async && !m_log_queue->full())
    {
        //异步写入阻塞队列
        m_log_queue->push(log_str);
    }
    else
    {
        //同步写入
        m_mutex.lock();
        fputs(log_str.c_str(), log_file);
        m_mutex.unlock();
    }
    va_end(valst);
}
void log::flush(void)
{
    m_mutex.lock();
    //强制刷新写入流缓冲区
    fflush(log_file);
    m_mutex.unlock();
}